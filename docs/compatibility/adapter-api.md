...
The test-kit design must also define native SQL optimization and dialect
validation conformance before production adapters claim execution
compatibility. Adapter execution SQL must be rendered from typed operation
payloads, not authored YAML strings or untyped raw SQL templates. Conformance
must reject untyped `select *` comparison plans, naive full-row joins,
unbounded source/target row movement into Core, hidden Python fallback, and
implicit partition/window filtering. Adapter-native SQL snapshots and dialect
syntax validation can catch renderer regressions, but they are not correctness
proof without semantic execution cases for the capabilities the adapter claims.
...
metadata fields, unsafe path-like step names, invalid later rendered steps, or
duplicate step names including case-insensitive output collisions after path
normalization, must fail with `RC_ADAPTER_OPERATION_RENDER_FAILED` before any
compiled SQL artifacts are written. The compiled SQL writer must revalidate the
same rendered-step shape at the publication boundary, because Core owns the
filesystem artifact safety guarantee even when a future helper, test kit, or
adapter repository hands `RenderedSql` to the writer directly.

Rendered SQL step `required_capabilities` are capability requirements, not
comments. Current Core `render_check_sql()` orchestration and
`recon compile --render-sql` validate and enforce step-level required
capabilities in addition to the compiled check plan requirements before
compiled SQL is published. Future shared adapter test kits, extracted adapter
repositories, and renderer registries must preserve that invariant and expand
conformance cases for supported step capabilities, unsupported,
not-implemented, unknown, or versioned capability states, malformed capability
declarations, and renderer-declared extra capabilities that were not present on
the compiled check plan. Unsupported step-level requirements must fail clearly
before SQL, execution results, evidence, or adapter test snapshots are
published.
...
adapter `type` enforcement before adapter resolution, and DSN component
redaction cases for username, password, host, path, query values,
percent-decoded values, and derived substrings, plus blocked compiled-check
...
flows, plus full-batch generated-artifact preflight cases proving empty or
malformed rendered SQL write requests, later empty or malformed rendered SQL
requests, invalid later rendered steps, unsafe path segments, exact output
paths that already exist as directories or other non-files, and
case-insensitive duplicate output collisions leave no partial compiled SQL.
Required adapter capability semantic
jobs must install the relevant adapter package or optional extra and fail when
that dependency is missing or unimportable; required conformance coverage must
not silently skip through optional imports. If the test kit or external adapter
package claims execution compatibility, it must also include source/target data
privacy conformance for runtime diagnostics, raw adapter/database/runtime
exception text, rendered or executed query text, run results, evidence, failure
details, reports, logs, and test snapshots before compatibility is claimed. It
must also include native SQL optimization and dialect validation conformance for
the operations and capabilities it claims. Raw low-level exception text is not a
safe adapter diagnostic message unless it has been classified and sanitized
under the source/target data privacy policy.
...
| --- | --- | --- | --- | --- | --- |
| Profile selection and rendered connection config | Selected profile, selected target, referenced `legacy` and `warehouse` named connections, referenced `{{ env_var(...) }}` and bare `env_var(...)` fields, env-var defaults, missing referenced env vars, unsupported bare env-var expressions, embedded env-var calls, filters, Jinja statement/comment fragments such as `{% ... %}` and `{# ... #}`, unselected targets, and unreferenced connections. | Adapter-aware compile and current row-count and bounded local/dev grain-key safety execution load only selected/referenced connection config, fail missing or unsupported env-var/template syntax only for referenced config, never pass invalid template syntax through as literal config, and do not leak rendered values through diagnostics or generated artifacts. | Existing coverage in `tests/profiles/test_loader.py`, `tests/services/test_compile_service.py`, and current row-count and bounded local/dev grain-key safety runtime coverage in `tests/services/test_run_service.py`. | Gates debug/profile validation commands, broader adapter execution, and shared adapter test-kit profile fixtures. | Profile debug commands, profile schema versioning, and external test-kit fixtures are future work. |
...
| Adapter factory and resolution contract | Registered factory, unknown adapter type, empty factory result, malformed result, malformed diagnostic container, malformed diagnostic entries, invalid `Diagnostic` fields, factory exception, same-connection duplicate setup diagnostics, distinct source/target setup diagnostics, setup diagnostics alongside independent render diagnostics, and factory returns both adapter and diagnostics. | Factory resolution produces an adapter or structured diagnostics. Malformed or exception-raising factories fail with sanitized `RC_ADAPTER_RESOLUTION_FAILED`. Setup diagnostics block rendering, write no compiled SQL, de-duplicate repeated same-connection service diagnostics, preserve distinct connection diagnostics, and do not mask render diagnostics from otherwise resolvable contracts in the same compile invocation. | Existing coverage in `tests/adapters/test_registry.py` and `tests/services/test_compile_service.py`; current row-count and bounded local/dev grain-key safety runtime resolution coverage in `tests/services/test_run_service.py`. | Gates external adapter package APIs, adapter test-kit fixtures, and execution-time adapter resolution. | Connection pooling and production adapter package distribution are future work. |
...
| Runtime adapter lifecycle | Connection/open success and failure, execute success and failure, close success and failure, execute-plus-close failure, lifecycle diagnostics with unsafe rendered profile values, raw SQL, rendered SQL, relation data, database error text, DSN fragments, credentials, and tracebacks. | Core starts adapter lifecycle only after profile, factory, metadata, API, capability, placement, and privacy checks pass. Connection failures use sanitized `RC_ADAPTER_CONNECTION_FAILED`, execution failures use sanitized `RC_ADAPTER_QUERY_FAILED`, and close failures use sanitized `RC_ADAPTER_CLOSE_FAILED`. Opened adapters are closed, close failures do not hide a primary execution failure, and public diagnostics preserve safe actionable messages without exposing sensitive runtime payloads. | Current row-count and bounded local/dev grain-key safety runtime coverage in `tests/services/test_run_service.py` and CLI privacy coverage in `tests/cli/test_main.py`; future aggregate execution and shared adapter-test-kit coverage must add their own cases. | Gates later aggregate execution, debug connection commands, adapter test-kit snapshots, external adapter packages, and source/target privacy enforcement. | Connection pooling, long-lived sessions, transaction management, query endpoint execution, native error disclosure, and shared test-kit lifecycle snapshots remain separately gated future work. |
...
| SQL comparison semantics for future adapter test kit | Null-safe equality, key-diff semantics, nullable grouped aggregate keys, no implicit type coercion, representative cross-type values, empty relation type mismatches, aggregate input and result type mismatches, boolean aggregate inputs, unsigned large integers, exact numeric aggregate preservation, empty aggregate result semantics, unsupported-capability behavior, and required adapter dependency installation in semantic CI. | A future shared test kit must make comparison semantics executable across adapters before external adapter compatibility is claimed. Unsupported or unsafe behavior must fail clearly rather than silently relying on dialect casts, fallback behavior, or optional-import skips in required conformance jobs. | Planned shared adapter test-kit coverage; current in-core DuckDB renderer tests cover the rendering subset and CI-gated executable DuckDB semantic cases through the optional DuckDB extra. | Gates `recon-adapter-testkit`, `recon-duckdb` package split, and production adapter compatibility claims. | Execution is staged through row count, grain-key checks, and aggregates; current execution is limited to same-context DuckDB row-count and grain-key safety checks allowed by the internal bounded local/dev scan guard. |
| Native SQL optimization and dialect validation conformance | Typed operation rendering, adapter-native SQL snapshots, optional dialect syntax validation, no direct YAML-to-SQL compilation, no untyped `select *` comparison plans, no naive full-row joins, no unbounded row movement into Core, no hidden Python fallback, no implicit partition/window filters, and no blind transpilation compatibility claims. | Production execution compatibility requires SQL that preserves Core typed-plan semantics and the locked execution placement. Syntax validation can catch dialect mistakes, but semantic execution tests remain the compatibility proof. Unsupported or unsafe optimization paths must fail with structured diagnostics instead of falling back to expensive or misleading comparison strategies. | Planned shared adapter test-kit coverage; current in-core rendering remains relation-backed and does not claim production adapter execution conformance. | Gates production adapter execution compatibility, native SQL renderer claims, and future warehouse-scale adapter support. | Row-hash comparison, materialization, external comparison engines, selector/window scope, and large failure-detail exports remain separately gated surfaces. |
...
Any public or shared rendering orchestration entry point that accepts both a
resolved adapter and an explicit renderer must validate adapter API
compatibility and must validate that the renderer declares a non-empty
`adapter_type` equal to the resolved adapter type before rendering. The
...
directories or empty `rendering.sql_paths`. Malformed non-empty `RenderedSql`
steps are also publication failures: blank SQL, blank operation types, malformed
required capability declarations, unsafe step names, and duplicate step names
must fail before files or directories are published. Core must validate the full
rendered SQL batch and preflight output directories and file paths before
creating the first compiled SQL directory or file. Future adapter test kits and
external adapter repositories must include empty or malformed direct-writer
requests, later empty or malformed batch requests, invalid later rendered
steps, unsafe step names, and duplicate or case-insensitive output collisions
as no-partial-artifact conformance cases.
