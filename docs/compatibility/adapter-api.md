...
| Runtime adapter lifecycle | Connection/open success and failure, execute success and failure, close success and failure, execute-plus-close failure, lifecycle diagnostics with unsafe rendered profile values, raw SQL, rendered SQL, relation data, database error text, DSN fragments, credentials, and tracebacks. | Core starts adapter lifecycle only after profile, factory, metadata, API, capability, placement, and privacy checks pass. Connection failures use sanitized `RC_ADAPTER_CONNECTION_FAILED`, execution failures use sanitized `RC_ADAPTER_QUERY_FAILED`, and close failures use sanitized `RC_ADAPTER_CLOSE_FAILED`. Opened adapters are closed, close failures do not hide a primary execution failure, and public diagnostics preserve safe actionable messages without exposing sensitive runtime payloads. | Planned runtime adapter tests for execution phases that open adapters; current compile-only tests remain limited to resolution, metadata, capabilities, and rendering. | Gates row-count execution, later key/aggregate execution, debug connection commands, adapter test-kit snapshots, external adapter packages, and source/target privacy enforcement. | Connection pooling, long-lived sessions, transaction management, query endpoint execution, and native error disclosure remain separately gated future work. |
