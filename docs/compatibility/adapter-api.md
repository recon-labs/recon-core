...
| --- | --- | --- | --- | --- | --- |
| Profile selection and rendered connection config | Selected profile, selected target, referenced `legacy` and `warehouse` named connections, referenced `{{ env_var(...) }}` and bare `env_var(...)` fields, env-var defaults, missing referenced env vars, unsupported bare env-var expressions, embedded env-var calls, filters, Jinja statement/comment fragments such as `{% ... %}` and `{# ... #}`, unselected targets, and unreferenced connections. | Adapter-aware compile and current row-count execution load only selected/referenced connection config, fail missing or unsupported env-var/template syntax only for referenced config, never pass invalid template syntax through as literal config, and do not leak rendered values through diagnostics or generated artifacts. | Existing coverage in `tests/profiles/test_loader.py`, `tests/services/test_compile_service.py`, and current row-count runtime coverage in `tests/services/test_run_service.py`. | Gates debug/profile validation commands, broader adapter execution, and shared adapter test-kit profile fixtures. | Profile debug commands, profile schema versioning, and external test-kit fixtures are future work. |
...
| Adapter factory and resolution contract | Registered factory, unknown adapter type, empty factory result, malformed result, malformed diagnostic container, malformed diagnostic entries, invalid `Diagnostic` fields, factory exception, same-connection duplicate setup diagnostics, distinct source/target setup diagnostics, setup diagnostics alongside independent render diagnostics, and factory returns both adapter and diagnostics. | Factory resolution produces an adapter or structured diagnostics. Malformed or exception-raising factories fail with sanitized `RC_ADAPTER_RESOLUTION_FAILED`. Setup diagnostics block rendering, write no compiled SQL, de-duplicate repeated same-connection service diagnostics, preserve distinct connection diagnostics, and do not mask render diagnostics from otherwise resolvable contracts in the same compile invocation. | Existing coverage in `tests/adapters/test_registry.py` and `tests/services/test_compile_service.py`; current row-count runtime resolution coverage in `tests/services/test_run_service.py`. | Gates external adapter package APIs, adapter test-kit fixtures, and execution-time adapter resolution. | Connection pooling and production adapter package distribution are future work. |
...
| Runtime adapter lifecycle | Connection/open success and failure, execute success and failure, close success and failure, execute-plus-close failure, lifecycle diagnostics with unsafe rendered profile values, raw SQL, rendered SQL, relation data, database error text, DSN fragments, credentials, and tracebacks. | Core starts adapter lifecycle only after profile, factory, metadata, API, capability, placement, and privacy checks pass. Connection failures use sanitized `RC_ADAPTER_CONNECTION_FAILED`, execution failures use sanitized `RC_ADAPTER_QUERY_FAILED`, and close failures use sanitized `RC_ADAPTER_CLOSE_FAILED`. Opened adapters are closed, close failures do not hide a primary execution failure, and public diagnostics preserve safe actionable messages without exposing sensitive runtime payloads. | Current row-count runtime coverage in `tests/services/test_run_service.py` and CLI privacy coverage in `tests/cli/test_main.py`; future key/aggregate execution and shared adapter-test-kit coverage must add their own cases. | Gates later key/aggregate execution, debug connection commands, adapter test-kit snapshots, external adapter packages, and source/target privacy enforcement. | Connection pooling, long-lived sessions, transaction management, query endpoint execution, native error disclosure, and shared test-kit lifecycle snapshots remain separately gated future work. |
