...
comments. Before a shared adapter test kit, extracted adapter repository, or
future renderer registry claims compatibility, orchestration must validate and
enforce step-level required capabilities in addition to the compiled check plan
requirements. Conformance cases must include supported step capabilities,
unsupported, not-implemented, unknown, or versioned capability states, and
renderer-declared extra capabilities that were not present on the compiled
...
requests, invalid later rendered steps, unsafe path segments, and
