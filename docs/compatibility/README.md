...
| Adapter capabilities | Support-state validation exists; the in-core DuckDB local adapter declares the current rendering, row-count execution, and bounded local/dev grain-key safety execution subset. |
...
| Check-engine boundary and result model | Implemented as a pre-alpha in-memory boundary for already compiled checks, including relation-backed same-context DuckDB `row_count_diff` and bounded local/dev grain-key safety execution; generated run-result artifacts, evidence, aggregate execution, and row-level value comparison remain future work. |
