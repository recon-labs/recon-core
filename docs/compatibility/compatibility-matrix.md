...
| Rendered SQL step capability enforcement | Planned and gated for shared renderer/test-kit surfaces | Before a shared renderer helper, renderer registry, adapter test kit, extracted `recon-duckdb`, production adapter package, or external adapter compatibility claim treats rendered SQL as compatible, orchestration must enforce `RenderedSql.required_capabilities` in addition to compiled check-plan requirements. Unsupported, not-implemented, unknown, versioned, malformed, or extra step-level capability declarations must fail clearly before SQL artifacts, run results, evidence, or adapter test snapshots are published. |
| Renderer Output And Artifact Publication Conformance Gate | Required for shared renderer, test-kit, adapter, and future generated-artifact surfaces | Before a shared adapter test kit, extracted `recon-duckdb`, production adapter package, renderer registry, execution path, or external adapter compatibility claim treats rendered SQL or generated artifacts as compatible, `docs/compatibility/adapter-api.md` must map renderer-output and publication rows to tests. The gate requires non-empty renderer output, exported writer rejection of empty or malformed rendered SQL requests, batch rejection of later empty or malformed rendered SQL requests, malformed-output rejection, step-level capability enforcement, and all-or-nothing artifact publication before SQL artifacts, run results, evidence, reports, logs, snapshots, or state are published. Core owns filesystem publication safety; adapter repositories prove valid, capability-coherent renderer output and must not bypass Core preflight. |
