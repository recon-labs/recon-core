...
| Adapter test kit | Planned | No test-kit package or workflow exists yet; before it is created or split into a repository, it must define a SQL comparison conformance matrix for null-safe equality, key-diff semantics, grouped nullable keys, cross-type comparisons, key/group and aggregate-value type-mismatch failures including empty-relation cases, no cross-type grouped-key coalescing, and unsupported-capability behavior. |
