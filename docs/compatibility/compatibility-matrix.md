...
| Profile and secret handling | Implemented for adapter-aware compile | Selected-target rendering, referenced-connection filtering, env-var rendering for non-routing connection config, literal non-empty adapter `type` enforcement, templated `type` rejection before adapter resolution, unsupported-template rejection, profile-backed adapter diagnostic suppression including adapter API compatibility and render-phase diagnostics, unsafe adapter-provided diagnostic code replacement, `rendering.adapter_type` redaction, non-string rendered-value redaction including separatorless diagnostic-code embeddings such as `RCsuper-secretLEAK` and `RC12LEAK`, numeric `line`/`column` diagnostic fields, and short numeric rendered scalars such as port values in diagnostic codes, diagnostic text, unsafe resource metadata, and `rendering.adapter_type`, including integer-equivalent formatted variants such as `12.0`, `+12`, and `1.2e1`, and same adapter connection-context enforcement are implemented for `--render-sql`; run-time profile loading, debug/profile validation commands, and shared conformance tests are future work. |
...
| Adapter test kit | Planned | No test-kit package or workflow exists yet; before it is created or split into a repository, it must define adapter API conformance tests for registry/factory resolution, including empty or malformed factory results, malformed factory diagnostic payloads including invalid `Diagnostic` field values, factories that return both adapters and diagnostics, sanitized factory exceptions, missing or invalid adapter API version declarations, invalid or exception-raising `adapter_type` metadata, sanitized capability declaration exceptions, malformed capability support states, adapter API compatibility diagnostics, render-phase diagnostics, profile-rendering behavior including literal adapter `type` enforcement and templated `type` rejection before adapter resolution, field-by-field adapter diagnostic redaction across diagnostic code, message, hint, path, `resource_type`, `resource_name`, `line`, `column`, `rendering.adapter_type`, and future structured diagnostic fields, separatorless diagnostic-code embeddings such as `RCsuper-secretLEAK` and `RC12LEAK`, case-variant and non-string rendered-config redaction, short numeric rendered-scalar cases such as `port: 12` and formatted equivalents such as `12.0`, `+12`, and `1.2e1` in diagnostic codes, text fields, unsafe resource metadata, numeric `line`/`column`, and `rendering.adapter_type`, safe non-empty diagnostic messages, adapter setup failures that write no compiled SQL, mark affected compiled checks blocked, de-duplicate repeated same-connection service diagnostics, and preserve distinct source/target connection diagnostics in service output and blocked compiled-check artifacts, core render-sql compile-validation blocked-metadata integration cases when the test kit drives core compile flows, and empty or malformed renderer output failures, plus a SQL comparison conformance matrix for null-safe equality, key-diff semantics, grouped nullable keys, cross-type comparisons, key/group and aggregate-value type-mismatch failures including empty-relation, boolean aggregate, same-type unsupported/non-numeric aggregate-input, unsigned-large-integer aggregate cases such as DuckDB `UHUGEINT`, exact numeric aggregate preservation for large integers and decimals, empty aggregate result semantics where engines such as DuckDB return `NULL` for `sum` on empty groups rather than zero, no cross-type grouped-key coalescing, same-context rendering requirements, and unsupported-capability behavior. Invalid diagnostic field value cases must include string severities instead of `DiagnosticSeverity`, empty or non-string `code` or `message`, non-string optional context fields, and non-integer `line` or `column` values. |
