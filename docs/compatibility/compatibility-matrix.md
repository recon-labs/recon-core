...
| Adapter test kit | Planned | No test-kit package or workflow exists yet; before it is created or split into a repository, it must define adapter API conformance tests for registry/factory resolution, including empty factory results, plus a SQL comparison conformance matrix for null-safe equality, key-diff semantics, grouped nullable keys, cross-type comparisons, key/group and aggregate-value type-mismatch failures including empty-relation, boolean aggregate, and same-type unsupported/non-numeric aggregate-input cases, exact numeric aggregate preservation for large integers and decimals, no cross-type grouped-key coalescing, and unsupported-capability behavior. |
