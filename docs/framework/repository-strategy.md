...
2. Split `recon-postgres`, `recon-snowflake`, and `recon-adapter-testkit` after typed check plans, adapter API versioning, and shared adapter tests stabilize. Shared adapter tests must include profile-rendering and diagnostic-redaction conformance, including sanitized adapter factory exceptions and sanitized capability declaration exceptions, before any split repo claims compatibility.
