...
connections to resolve to the same adapter connection config; distinct configs
are blocked rather than implicitly bridged. If any check produces a rendering
diagnostic, compile writes no SQL files for that adapter-aware invocation and
marks checks `blocked` or `failed` rather than leaving them `not_rendered`.
