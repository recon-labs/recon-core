...
| Scan-budget and query-plan safety | Full scan allowed under explicit budget, bounded local/dev relation-backed fixture, production unknown estimate, unavailable estimate, unsupported or malformed estimate capability, unsafe executing profile/analyze preflight, over-budget estimate. | Key-safety checks may execute only when scan scope and budget status are explicit. Production unknown, unavailable, unsupported, malformed, unsafe, or over-budget outcomes are `not_executable`, not data failures. Bounded local/dev execution requires explicit local, relation-backed, bounded classification. | 7.3 tests for allowed bounded local/dev execution, explicit within-budget classification, production unknown estimate, unavailable/unsupported/malformed estimate capability, unsafe preflight, over-budget estimate, and no scan after a hard budget block. | Gate 4L, comparison placement gate, source/target privacy gate. | Full user-facing budget settings, contract-level budget policy, and production adapter scan-estimation conformance remain later work. |
