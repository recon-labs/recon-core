...
- bounded local/dev fixture exceptions are allowed only if the internal scan
  guard classifies the input as local, relation-backed, file-size bounded with
  no retained DuckDB sidecars, and backed by local DuckDB base tables rather
  than views or external data sources,
...
| Scan-budget allowed path | Bounded local/dev relation-backed fixture, scan classification present, local DuckDB base-table metadata, no retained DuckDB sidecars, internal bounded-local budget status. | A key-safety check may execute only when scope and budget status are explicit and the internal scan guard classifies the context as local, relation-backed, file-size bounded with no retained DuckDB sidecars, and backed by local DuckDB base tables. M7.3 has no production within-budget execution path. | Add scan-budget policy tests for allowed bounded local/dev execution, local base-table metadata, recorded bounded-local classification, absent retained sidecars, and no new contract YAML settings. | Gate 4L. | Full general user-facing budget settings and production within-budget execution remain future work. |
| Scan-budget fail-closed path | Retained DuckDB sidecars, DuckDB views, externally backed relations, missing local catalog metadata, production estimate present, production unknown estimate, unavailable estimate, unsupported estimation/capability, malformed estimate, executing profile/analyze as unsafe preflight, over-budget preflight if a future or adapter-provided policy reports it. | Retained DuckDB sidecars, DuckDB views, externally backed relations, and missing local base-table metadata do not satisfy the bounded local/dev exception. Production estimate-dependent paths are `not_executable` in M7.3 because no user-facing budget settings or production adapter scan-estimation compatibility is locked yet. Unsafe-estimate and over-budget outcomes are execution-policy outcomes, not data failures. Recon must not run the scan, must not report mismatch evidence, and must not use executing profile/analyze as safe preflight by default. | Add scan-budget tests for retained DuckDB sidecars, DuckDB views, or externally backed relations blocked before adapter setup, production estimate present but blocked, unknown production estimate, unsupported estimate capability, malformed estimate, unsafe executing preflight, over-budget signals when representable, and adapter estimate support states. | Gate 4L, Gate 4I, Gate 6. | Adapter test-kit rows for production scan-estimation compatibility remain future work. |
...
| Bounded local/dev relation-backed fixture | May execute only when the internal scan guard classifies it as local, relation-backed, file-size bounded with no retained DuckDB sidecars, and backed by local DuckDB base tables. | Add local/dev exception test. |
| Retained DuckDB sidecar, view, or externally backed relation under a small local file | `not_executable`; the bounded local/dev exception fails closed before adapter setup. | Add scan-budget fail-closed test. |
...
Given the internal scan guard classifies the execution context as local,
relation-backed, file-size bounded with no retained DuckDB sidecars, and backed
by local DuckDB base tables.
...
| Gate 4L: scan budget and query-plan safety | Satisfied for Step 5 bounded policy. | Matrix rows cover bounded local/dev fixture execution as the only M7.3 allowed scan path, local DuckDB base-table metadata as part of bounded classification, DuckDB views and externally backed relations failing closed, production estimate-present paths as `not_executable` until M8 settings, scan blocked over budget as `not_executable` when representable, production unknown estimate as `not_executable`, unsupported estimation/capability as `not_executable`, unsafe executing profile/analyze rejection, and future user-facing budget settings boundary. |
...
- bounded local/dev fixture execution works only when the internal scan guard
  classifies the input as local, relation-backed, and bounded,
