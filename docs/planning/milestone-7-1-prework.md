...
| Compiled typed operation payload is malformed or contains fields not valid for its known operation type. | Runtime artifact-invalid diagnostic; dispatch does not run. | Loader validation test for malformed operation payload and unexpected known-operation fields. |
